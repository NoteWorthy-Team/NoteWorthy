...
class userInfo {
  ...
  let friendOne = new friendInfo(1,"csc309", './img/csc309.jpg', '');
  let friendTwo = new friendInfo(2,"emptyuser", './img/emptyuser.jpg','');

  let sampleFriendList = [];
  sampleFriendList.push(friendOne)
  sampleFriendList.push(friendTwo)

  let favouriteAlbum = new albumInfo(0, " Please Hammer Don’t Hurt ‘Em ", './img/please_hammer_dont_hurt_em.jpg');
  let sampleFavAlbumList = [];
  sampleFavAlbumList.push(favouriteAlbum);

  let nothworthyReview = new reviewData( new Date(2018, 11, 24),1, "Bahen… ",'./img/bahen....jpg',
  ...
  "Please Hammer Don’t Hurt ‘Em ",'./img/please_hammer_dont_hurt_em.jpg',
  ...
  let collectionAlbum = new albumInfo(1, "Bahen...", './img/bahen....jpg');
  ...
  let helpAlbum = new albumInfo(2,"Help", './img/help.jpg');
  let tolistenList = [];
  tolistenList.push(helpAlbum)

  let sampleUser = new userInfo(0,"tonybaloney",'./img/tonybaloney.jpg',
      ...
      if( user.userCollections.length != 0) {
        for(let i = 0; i< 3 && i < user.userCollections.length ; i++)
        {
          ...
          for(let i =0; i < 4; i++) {
            ...
            {
              currentAlbumCoverImg.src = './img/grey.jpg'
            }

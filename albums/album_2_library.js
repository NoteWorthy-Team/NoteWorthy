...
const HelpAlbum = new album(2, "Hep!", '../samples/sample_album_art/help.jpg',
  'The Beatles', 'George Martin', 1965, 'Rock', 'Parlophone', '33:44',
  helpTrackList, getAverageRating(helpReviewList), helpReviewList);
...
// Event lister
  ...
  function displayAlbumInfo(album) {
    ...
    function styleFavouriteButton() { 
      ...
      const albumID = HelpAlbum.albumId;
  ...
  function displayReviews(reviews) { // Display some recent review
    if( reviews.length != 0) {
      for(let i = 0; i< 3 && i < reviews.length; i++)
      {
        ...
        // values here
        const reviewAlbum = HelpAlbum;
        const reviewUser = cscUser;
...
// The review list will be returned from a server call
  ...
  function submitNewReview(e)
  {
      ...
      HelpAlbum.reviews.push(newReview)
      HelpAlbum.avgRating = getAverageRating( HelpAlbum.reviews)
      ...
      displayReviews(HelpAlbum.reviews);
      displayAverageRating(HelpAlbum.avgRating)
